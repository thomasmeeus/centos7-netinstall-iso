...
cp -r hosts/${hostname}.ks target-iso/ks.cfg                      # add your kickstart file & defina a network repo for packages
...
rm -rf target-iso/isolinux/upgrade.img

cd target-iso/
mkisofs -R -no-emul-boot -boot-load-size 4 -boot-info-table -joliet-long -o ../output/centos72-${hostname}.iso -b isolinux/isolinux.bin -c isolinux/boot.cat -V "CENTOS7 KS" .
cd ../
rm -rf target-iso/*
